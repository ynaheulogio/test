...
class Playon extends Eden {
	...
	public function setPaths() {
		$this->registry()
			...
			->set('path', 'page', 		$this->_root.'/playon/page')
			->set('path', 'template', 	$this->_root.'/playon/template')
	...
	public function setRequest() {
		...
		if(!$page) { 
			...
			while(count($buffer) > 1) {
				...
				$class = 'Playon_Page'.str_replace(' ', '_', $parts);
	...
	public function template($file, array $data = array()) {
		Playon_Error::i()->argument(1, 'string');
	...
	public function error($error, $event, $type, $level, 
		...
		echo Eden_Template::i()
			...
			->set('line', $line)
			->set('message', $message)
			->parsePhp(dirname(__FILE__).'/playon/error.phtml');
	}
